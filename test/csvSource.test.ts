 ...
 * retries doesn't make this suite slow. A single large advance (well beyond the ~3.9s maximum
 * cumulative backoff across all 3 real retry attempts) flushes every pending sleep in order;
...
describe('fetchYearContentHash', () => {
    ...
    it('gives up and throws after exhausting all retry attempts against a persistent 500', async () => {
        ...
        expect(fetchMock).toHaveBeenCalledTimes(3); // MAX_RETRY_ATTEMPTS
    });
